...
test('Swagger와 health, CRUD, multipart, SSE를 제공한다', async (t) => {
  ...
  response = await fetch(`${baseUrl}/docs-json`);
  assert.equal(response.status, 200);
  const document = await response.json();
  assert.match(document.openapi, /^3\./);
  for (const route of ['/health', '/items', '/items/{id}', '/upload', '/events']) {
    assert.ok(document.paths[route], `OpenAPI path missing: ${route}`);
  }
});
