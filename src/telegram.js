...
export function initTelegramBot() {
  ...
  bot.setMyCommands([
    ...
    { command:'profile', description:'Execution Profile 선택' },
  ...
  async function processPromptJob(chatId,userId,userText,attachedFiles=[]){
    ...
    const contextPackage=ContextManager.buildContextPackage(activeSession.id);const history=contextPackage.messages.filter(m=>m.id!==userMessageId).slice(-10);const contextBlock=history.length?`[이전 대화 기록 / Context]\n${history.map(m=>`${m.role==='user'?'User':'Assistant'}: ${m.text}`).join('\n\n')}`:'';
    const parts=[];if(memoryBlock)parts.push(memoryBlock);if(contextPackage.rollingSummary)parts.push(`[대화 요약]\n${contextPackage.rollingSummary}`);if(contextBlock)parts.push(contextBlock);parts.push(promptWithAttachments||canonicalUserText);const finalPrompt=parts.join('\n\n');
    ...
    try{statusMsg=await JobStatusRenderer.sendInitialStatus(bot,chatId,{sessionId:activeSession.id,sessionTitle:activeSession.title,provider:activeSession.active_provider,model:activeSession.active_model});const response=await queueManager.enqueueJob({sessionId:activeSession.id,sessionTitle:activeSession.title,provider:activeSession.active_provider,model:activeSession.active_model,prompt:finalPrompt,profile:activeSession.execution_profile,onStatusUpdate:(status,elapsed)=>{if(statusMsg)JobStatusRenderer.updateStatus(bot,chatId,statusMsg.message_id,{sessionId:activeSession.id,sessionTitle:activeSession.title,provider:activeSession.active_provider,model:activeSession.active_model},status,elapsed);}});SessionManager.saveMessage({sessionId:activeSession.id,role:'assistant',text:response,provider:activeSession.active_provider,model:activeSession.active_model});TitleService.autoGenerateTitleIfEligible(activeSession.id,userText||'첨부 파일 대화',response).catch(()=>{});for(const chunk of splitMessage(response))await bot.sendMessage(chatId,chunk,{parse_mode:'Markdown'}).catch(()=>bot.sendMessage(chatId,chunk));}
    catch(err){console.error(`[Job Error] ${err.message}`);if(statusMsg)JobStatusRenderer.updateStatus(bot,chatId,statusMsg.message_id,{sessionId:activeSession.id,sessionTitle:activeSession.title,provider:activeSession.active_provider,model:activeSession.active_model},'FAILED');await bot.sendMessage(chatId,`❌ 작업 실패:\n${err.message}`);}
  }
