...
import { handleStatusCommand } from './telegram/commands/status.js';
import { handleSystemCommand, handleSystemCallback } from './telegram/commands/system.js';
import { handleClearCommand } from './telegram/commands/clear.js';
import { handleUsageCommand, handleUsageCallback } from './telegram/commands/usage.js';
...
export function initTelegramBot() {
  ...
  bot.on('callback_query', safeHandler('callback_query', async (q) => {
    ...
    const data = q.data || '';
    if(data.startsWith('mcp_'))return handleMcpCallback(bot,q);if(data.startsWith('skills_'))return handleSkillsCallback(bot,q);if(data.startsWith('deploy_'))return handleDeployCallback(bot,q);if(data.startsWith('usage_'))return handleUsageCallback(bot,q);if(data.startsWith('system_'))return handleSystemCallback(bot,q);if(data.startsWith('preview_'))return handlePreviewCallback(bot,q);if(isSessionsCallbackData(data))return handleSessionsCallback(bot,q);if(data.startsWith('model_'))return handleModelCallback(bot,q);if(data.startsWith('providers_'))return handleProvidersCallback(bot,q);if(data.startsWith('profile_'))return handleProfileCallback(bot,q);if(data.startsWith('settings_'))return handleSettingsCallback(bot,q);if(data.startsWith('server_'))return handleServersCallback(bot,q);if(data.startsWith('job_cancel'))return handleJobCancelCallback(bot,q);if(data.startsWith('schedule_'))return handleScheduleCallback(bot,q);
  }));
