...
import { handleSettingsCommand, handleSettingsCallback } from './telegram/commands/settings.js';
...
export function initTelegramBot() {
  ...
  bot.on('message',async msg=>{const chatId=msg.chat.id,from=msg.from,text=msg.text?.trim();if(!isAuthorizedUser(from)||!text)return;const userId=from.id;
    ...
    if(text==='/new'){await handleNewCommand(bot,msg);return;}if(text==='/sessions'){await handleSessionsCommand(bot,msg);return;}if(text.startsWith('/rename')){await handleRenameCommand(bot,msg,text.replace(/^\/rename\s*/,''));return;}if(text==='/model'){await handleModelCommand(bot,msg);return;}if(text==='/providers'){await handleProvidersCommand(bot,msg);return;}if(text==='/profile'){await handleProfileCommand(bot,msg);return;}if(text==='/settings'){await handleSettingsCommand(bot,msg);return;}if(text.startsWith('/servers')){await handleServersCommand(bot,msg,text.replace(/^\/servers\s*/,''));return;}if(text==='/stop'){await handleStopCommand(bot,msg);return;}if(text==='/queue'){await handleQueueCommand(bot,msg);return;}if(text==='/compact'){await handleCompactCommand(bot,msg);return;}if(text==='/files'){await handleFilesCommand(bot,msg);return;}if(text.startsWith('/download')){await handleDownloadCommand(bot,msg,text.replace(/^\/download\s*/,''));return;}if(text.startsWith('/memory')){await handleMemoryCommand(bot,msg,text.replace(/^\/memory\s*/,''));return;}if(text.startsWith('/schedule')){await handleScheduleCommand(bot,msg,text.replace(/^\/schedule\s*/,''));return;}await processPromptJob(chatId,userId,text,[]);
  });
