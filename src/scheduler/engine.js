...
class SchedulerEngine {
  ...
  reconcileMissedRuns() {
    const db = getDb();
    const overdue = db.prepare(`SELECT * FROM schedules WHERE enabled=1 AND next_run_at IS NOT NULL AND next_run_at < ?`).all(new Date(Date.now() - 120000).toISOString());
  ...
  async tick() {
    const due = getDb().prepare(`SELECT * FROM schedules WHERE enabled=1 AND next_run_at IS NOT NULL AND next_run_at <= ? ORDER BY next_run_at ASC LIMIT 20`).all(new Date().toISOString());
  ...
  async runNow(scheduleId, userId) {
    const schedule = getDb().prepare(`SELECT * FROM schedules WHERE id=? AND user_id=? AND kind='USER'`).get(scheduleId, userId);
    if (!schedule) throw new Error('스케줄을 찾을 수 없습니다.');
    if (this.runningSchedules.has(schedule.id)) {
      this.recordRun(schedule.id, ScheduleRunStatus.SKIPPED, { errorMessage: 'Manual run overlap — SKIP' });
      return { skipped: true };
    }
    this.runningSchedules.add(schedule.id);
    try { await this.execute(schedule); return { skipped: false }; }
    finally { this.runningSchedules.delete(schedule.id); }
  }
  ...
  async execute(schedule) {
    ...
    try {
      const output = schedule.kind === 'SYSTEM' ? await this.executeSystem(schedule) : await this.executeUser(schedule, runId);
      ...
      db.prepare(`UPDATE schedule_runs SET status='COMPLETED',finished_at=datetime('now'),output_text=?,output_summary=?,duration_ms=? WHERE id=?`)
        .run(safeOutput, summarize(safeOutput), Date.now() - started, runId);
      if (schedule.kind === 'USER') await this.notify(schedule, `⏰ **예약 작업 완료**\n\n**${schedule.name}**\n\n${safeOutput.slice(0, 3500)}`);
    } catch (error) {
      const safeError = redactSecrets(error.message || error);
      const status = error.code === 'QUEUE_GRACE_EXCEEDED' ? 'SKIPPED' : 'FAILED';
      db.prepare(`UPDATE schedule_runs SET status=?,finished_at=datetime('now'),error_message=?,duration_ms=? WHERE id=?`)
        .run(status, safeError.slice(0, 2000), Date.now() - started, runId);
      if (schedule.kind === 'USER') {
        const title = status === 'SKIPPED' ? '⏭️ **예약 작업 스킵됨**' : '❌ **예약 작업 실패**';
        await this.notify(schedule, `${title}\n\n**${schedule.name}**\n${safeError}`);
      }
    }
  }
  ...
  async executeUser(schedule, runId) {
    const db = getDb();
    const executionSession = SessionManager.createSession(schedule.user_id, {
      title: `__schedule_run__:${schedule.id}:${runId}`,
      provider: schedule.provider,
      model: schedule.model,
      profile: schedule.execution_profile,
      isSystem: true,
      status: 'ARCHIVED'
    });
    const executionSessionId = executionSession.id;

    try {
      const memory = MemoryManager.getMemoryForPrompt();
      const prompt = [memory, `[예약 작업 / 일회성 독립 실행]\n${schedule.prompt}`].filter(Boolean).join('\n\n');
      const queueGraceMs = Math.max(1000, Number(process.env.SCHEDULER_QUEUE_GRACE_SECONDS || 30) * 1000);
      const jobPromise = queueManager.enqueueJob({
        ...
        profile: schedule.execution_profile,
        type: 'SCHEDULER',
        timeoutMs: schedule.timeout_seconds * 1000,
        queueGraceMs
      });
      if (jobPromise.jobId) db.prepare('UPDATE schedule_runs SET job_id=? WHERE id=?').run(jobPromise.jobId, runId);
      return await jobPromise;
    } finally {
      db.prepare(`UPDATE sessions SET status='DELETED',deleted_at=datetime('now'),updated_at=datetime('now') WHERE id=? AND is_system=1`).run(executionSessionId);
    }
  }
  ...
  advanceSchedule(schedule, from) {
    ...
    if (schedule.schedule_type === 'ONCE') {
      db.prepare(`UPDATE schedules SET enabled=0,last_run_at=datetime('now'),next_run_at=NULL,updated_at=datetime('now') WHERE id=?`).run(schedule.id);
      return;
    }
  ...
  recordRun(scheduleId, status, { errorMessage = null } = {}) {
    getDb().prepare(`INSERT INTO schedule_runs(id,schedule_id,status,error_message,finished_at) VALUES(?,?,?,?,datetime('now'))`)
      .run(crypto.randomUUID(), scheduleId, status, errorMessage ? redactSecrets(errorMessage) : null);
  }

  async notify(schedule, text) {
    if (!this.bot || !schedule.user_id) return;
    const safe = redactSecrets(text);
    try { await this.bot.sendMessage(schedule.user_id, safe, { parse_mode: 'Markdown' }); }
