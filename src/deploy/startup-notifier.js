...
export async function checkAndNotifyStartup({ bot, ownerId, dataDir = process.env.DATA_DIR || '/data', repoPath = '/home/dev/workspace/agent-hub-core' } = {}) {
  ...
  let text = `${uiStatusIcon('success')} **[Agent Hub Core V2 · 2.0.0 LTS]** 환영합니다! 정상 기동 완료\n\n`;
  ...
  try {
    ...
    console.log(`[StartupNotifier] 기동 알림 발송 완료: commit=${currentCommit}, isNew=${isNewDeployment}`);
    sendSuccess = true;
  } catch (sendErr) {
    ...
    try {
      ...
      console.log(`[StartupNotifier] 기동 알림 fallback(일반 텍스트) 발송 완료`);
      sendSuccess = true;
    } catch (fallbackErr) {
  ...
  return { notified: sendSuccess, isNewDeployment, currentCommit, lastCommit };
}
