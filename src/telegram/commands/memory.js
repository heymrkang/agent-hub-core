...
export async function handleMemoryCommand(bot, msg, args = '') {
  ...
  try {
    if (!trimmedArgs) {
      ...
      let text = `🧠 **글로벌 장기 기억 (Global Memory)**\n\n`;
      text += `\`\`\`markdown\n${memoryContent}\n\`\`\`\n`;
      text += `📌 **사용법**:\n`;
    ...
    if (trimmedArgs.startsWith('add ')) {
      ...
      if (!entry) {
        await bot.sendMessage(chatId, '❌ 추가할 메모리 내용을 입력해주세요.\n예: `/memory add 프로젝트 빌드 명령어는 npm run build 이다.`', {
          parse_mode: 'Markdown'
        });
        return;
      }
      ...
      await bot.sendMessage(chatId, `✅ **메모리 추가 완료**\n\n- ${entry}`, { parse_mode: 'Markdown' });
      return;
    }
    ...
    if (trimmedArgs.startsWith('set ')) {
      ...
      await bot.sendMessage(chatId, `✅ **메모리 전체 수정 완료**`, { parse_mode: 'Markdown' });
      return;
    }

    if (trimmedArgs === 'clear') {
      MemoryManager.clearMemory();
      await bot.sendMessage(chatId, `🧹 **글로벌 메모리가 초기화되었습니다.**`);
      return;
    }

    await bot.sendMessage(chatId, '❌ 알 수 없는 메모리 옵션입니다.\n사용법: `/memory`, `/memory add <내용>`, `/memory clear`', {
      parse_mode: 'Markdown'
    });
  } catch (error) {
    console.error(`[Command /memory Error] ${error.message}`);
    await bot.sendMessage(chatId, `❌ 메모리 처리 실패: ${error.message}`);
  }
}
