...
export async function handleScheduleCommand(bot, msg, args = '') {
  ...
  try {
    ...
    await bot.editMessageText(body, {
      ...
      reply_markup: { inline_keyboard: [[{ text: '✅ 등록', callback_data: `schedule_confirm:${token}` }, { text: '❌ 취소', callback_data: `schedule_cancel:${token}` }]] }
    });
  } catch (error) {
...
export async function handleScheduleCallback(bot, q) {
  ...
  if (data.startsWith('schedule_confirm:')) {
    ...
    const s = createSchedule(userId, item.intent);
  ...
  if (data.startsWith('schedule_cancel:')) {
    pending.delete(data.split(':')[1]);
    return bot.editMessageText('스케줄 등록을 취소했습니다.', { chat_id: chatId, message_id: q.message.message_id });
  }
  ...
  const [action, id, pageRaw] = data.split(':');
  ...
  if (action === 'schedule_toggle') {
    getDb().prepare(`UPDATE schedules SET enabled=CASE enabled WHEN 1 THEN 0 ELSE 1 END,updated_at=datetime('now') WHERE id=?`).run(id);
    return showScheduleDetail(bot, chatId, getSchedule(id, userId), q.message.message_id, page);
  }
  if (action === 'schedule_delete') {
    getDb().prepare('DELETE FROM schedules WHERE id=?').run(id);
    return showScheduleList(bot, chatId, userId, q.message.message_id, page);
  }
  if (action === 'schedule_run') {
    await bot.editMessageText(`▶️ **수동 실행 요청됨**\n\n${escapeMd(s.name)}\n\n스케줄 활성/비활성 상태는 변경하지 않습니다.`, {
      chat_id: chatId, message_id: q.message.message_id, parse_mode: 'Markdown',
      reply_markup: { inline_keyboard: [[{ text: '🔙 상세', callback_data: `schedule_open:${s.id}:${page}` }]] }
    }).catch(() => {});
    schedulerEngine.runNow(id, userId).catch((error) => console.error(`[Schedule Manual Run] ${id}: ${error.message}`));
    return;
  }
...
function createSchedule(userId, i) {
  ...
  const next = computeNextRun(i.schedule_type, i.schedule_value, i.timezone, new Date());
  ...
  return getSchedule(id, userId);
}

function getSchedule(id, userId) {
  return getDb().prepare(`SELECT * FROM schedules WHERE id=? AND user_id=? AND kind='USER'`).get(id, userId);
}

async function showScheduleList(bot, chatId, userId, messageId = null, requestedPage = 0) {
  const db = getDb();
  const total = db.prepare(`SELECT COUNT(*) AS c FROM schedules WHERE user_id=? AND kind='USER'`).get(userId).c;
  const pageCount = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const page = Math.min(Math.max(0, requestedPage), pageCount - 1);
  const rows = db.prepare(`SELECT * FROM schedules WHERE user_id=? AND kind='USER' ORDER BY created_at DESC LIMIT ? OFFSET ?`).all(userId, PAGE_SIZE, page * PAGE_SIZE);
  ...
  rows.forEach((s, index) => {
    ...
    buttons.push([{ text: `${page * PAGE_SIZE + index + 1}. ${s.enabled ? '🟢' : '⚪'} ${truncate(s.name, 28)}`, callback_data: `schedule_open:${s.id}:${page}` }]);
  });

  if (pageCount > 1) {
    const nav = [];
    if (page > 0) nav.push({ text: '◀️ 이전', callback_data: `schedule_page:${page - 1}` });
    nav.push({ text: `${page + 1} / ${pageCount}`, callback_data: `schedule_page:${page}` });
    if (page < pageCount - 1) nav.push({ text: '다음 ▶️', callback_data: `schedule_page:${page + 1}` });
    buttons.push(nav);
  }
...
async function showScheduleDetail(bot, chatId, s, messageId, page) {
  const text = `🗓️ **예약 작업 상세**\n\n${s.enabled ? '🟢 활성' : '⚪ 비활성'} · **${escapeMd(s.name)}**\n\n• 방식: \`${s.schedule_type}\` / \`${escapeMd(s.schedule_value)}\`\n• Provider: \`${escapeMd(s.provider)}\`\n• Model: \`${escapeMd(s.model || 'CLI Default')}\`\n• Timezone: \`${escapeMd(s.timezone)}\`\n• Timeout: \`${s.timeout_seconds}s\`\n• 다음 실행: ${s.next_run_at ? `\`${escapeMd(formatDate(new Date(s.next_run_at), s.timezone))}\`` : '`없음`'}\n\n**작업**\n${escapeMd(s.prompt || '')}`;
  const buttons = [
    [{ text: s.enabled ? '⏸️ 끄기' : '▶️ 켜기', callback_data: `schedule_toggle:${s.id}:${page}` }, { text: '▶️ 지금 실행', callback_data: `schedule_run:${s.id}:${page}` }],
    [{ text: '📜 기록', callback_data: `schedule_history:${s.id}:${page}` }, { text: '🗑️ 삭제', callback_data: `schedule_delete:${s.id}:${page}` }],
    [{ text: '🔙 목록', callback_data: `schedule_back:${page}` }]
  ];
  return bot.editMessageText(text, { chat_id: chatId, message_id: messageId, parse_mode: 'Markdown', reply_markup: { inline_keyboard: buttons } }).catch(() => {});
}

async function showHistory(bot, chatId, s, messageId, page) {
