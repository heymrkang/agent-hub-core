...
function normalizeStatus(value) {
  return VALID_STATUSES.has(value) ? value : 'ACTIVE';
}

function shortTitle(title, max = 30) {
  const value = String(title || '새 채팅').replace(/\s+/g, ' ').trim();
  return value.length > max ? `${value.slice(0, max - 1)}…` : value;
}

function tabLabel(status, current) {
  const labels = { ACTIVE: '활성', ARCHIVED: '보관함', DELETED: '휴지통' };
...
async function renderSessions(bot, source, status = 'ACTIVE', page = 0) {
  ...
  let text = `📁 **세션 · ${statusNames[status]}**\n\n`;
  ...
  if (!pageItems.length) {
    ...
    text += `세션 ${sessions.length}개 · ${page + 1}/${totalPages} 페이지\n`;
  ...
  const keyboard = pageItems.map((session, index) => {
    ...
    const lock = session.title_locked ? ' 🔒' : '';
    return [{
      text: `${current ? '● ' : ''}${absoluteIndex}. ${shortTitle(session.title)}${lock}`,
      callback_data: `session_info:${session.id}:${status}:${page}`
    }];
  });

  if (totalPages > 1) {
    const nav = [];
    if (page > 0) nav.push({ text: '◀ 이전', callback_data: `session_page:${status}:${page - 1}` });
    nav.push({ text: `${page + 1} / ${totalPages}`, callback_data: 'session_noop' });
    if (page < totalPages - 1) nav.push({ text: '다음 ▶', callback_data: `session_page:${status}:${page + 1}` });
    keyboard.push(nav);
  }
  ...
  if (status === 'DELETED' && sessions.length > 0) {
    keyboard.push([{ text: `🗑 휴지통 비우기 (${sessions.length})`, callback_data: 'session_trash_confirm' }]);
  }

  keyboard.push([{ text: '＋ 새 세션 만들기 (/new)', callback_data: 'session_create_new' }]);
...
export async function handleSessionsCommand(bot, msg, status = 'ACTIVE', page = 0) {
  ...
  try {
    await renderSessions(bot, msg, status, page);
  } catch (error) {
...
async function showSessionDetail(bot, callbackQuery, sessionId, returnStatus = null, returnPage = 0) {
  ...
  if (!session || session.user_id !== userId || session.is_system) {
  ...
  let text = `📄 **${shortTitle(session.title, 50)}**\n\n`;
  ...
  if (session.title_locked) text += `제목: 사용자 지정 🔒\n`;

  const buttons = [];
  if (session.status === 'ACTIVE') {
    if (!isCurrent) buttons.push([{ text: '✓ 이 세션으로 전환', callback_data: `session_switch:${session.id}:${page}` }]);
    buttons.push([
      { text: '📦 보관', callback_data: `session_archive:${session.id}:${page}` },
      { text: '🗑 휴지통으로', callback_data: `session_delete:${session.id}:${page}` }
    ]);
  } else if (session.status === 'ARCHIVED') {
    buttons.push([{ text: '♻ 복구', callback_data: `session_restore:${session.id}:ARCHIVED:${page}` }]);
  } else if (session.status === 'DELETED') {
    buttons.push([{ text: '♻ 복구', callback_data: `session_restore:${session.id}:DELETED:${page}` }]);
    buttons.push([{ text: '🗑 영구 삭제', callback_data: `session_purge_confirm:${session.id}:${page}` }]);
  }
  ...
  await editMessageIfChanged(
    ...
    `⚠️ **휴지통 비우기**\n\n휴지통의 세션 **${count}개**를 영구 삭제합니다.\n대화 기록과 해당 세션의 첨부파일도 함께 제거되며 복구할 수 없습니다.`,
    {
      ...
      reply_markup: { inline_keyboard: [
        [{ text: `🗑 ${count}개 영구 삭제`, callback_data: 'session_trash_empty' }],
...
export async function handleSessionsCallback(bot, callbackQuery) {
  ...
  try {
    ...
    if (data.startsWith('session_switch:')) {
      ...
      const session = SessionManager.getSession(sessionId);
      await bot.answerCallbackQuery(callbackQuery.id, { text: `[${session?.title || '세션'}] (으)로 전환되었습니다.` });
    ...
    if (data.startsWith('session_purge_confirm:')) {
      ...
      await editMessageIfChanged(
        ...
        `⚠️ **세션 영구 삭제**\n\n**${shortTitle(session.title, 50)}**\n\n대화 기록과 해당 세션의 첨부파일을 영구 삭제합니다. 이 작업은 복구할 수 없습니다.`,
        {
          ...
          parse_mode: 'Markdown',
          reply_markup: { inline_keyboard: [
            [{ text: '🗑 영구 삭제', callback_data: `session_purge:${session.id}:${page}` }],
    ...
    if (data.startsWith('session_purge:')) {
      ...
      await bot.answerCallbackQuery(callbackQuery.id, { text: '세션을 영구 삭제했습니다.' });
      await renderSessions(bot, callbackQuery, 'DELETED', Number(page) || 0);
      return;
    }

    if (data === 'session_trash_confirm') {
      await confirmTrashEmpty(bot, callbackQuery);
      return;
    }

    if (data === 'session_trash_empty') {
      const count = SessionManager.emptyTrash(userId);
      await bot.answerCallbackQuery(callbackQuery.id, { text: `휴지통의 세션 ${count}개를 영구 삭제했습니다.` });
      await renderSessions(bot, callbackQuery, 'DELETED', 0);
      return;
    }
