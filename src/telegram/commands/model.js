...
export async function handleModelCommand(bot, msg) {
  ...
  try {
    ...
    const text = `🤖 **모델 및 프로바이더 설정**\n\n📌 **활성 세션**: **${active.title}**\n• 현재 Provider: \`${active.active_provider}\`\n• 현재 Model: \`${active.active_model || '기본 모델 (CLI Default)'}\`\n\n변경할 Provider를 선택하세요:`;
    const buttons = providers.map((p) => [{
      text: p === active.active_provider ? `🟢 ${p.toUpperCase()} (선택됨)` : `⚪ ${p.toUpperCase()}`,
...
async function renderModelsForProvider(bot, { chatId, messageId, userId, providerName }) {
  ...
  if (!models.length) {
    ...
    return editModelMessage(bot, chatId, messageId, text, {
      inline_keyboard: [
        [{ text: '🔄 지금 조회', callback_data: `model_refresh:${providerName}` }],
  ...
  buttons.push([
    ...
    { text: '🔙 Provider 목록', callback_data: 'model_back_to_providers' }
  ]);
...
async function showModelsForProvider(bot, q, providerName) {
  await bot.answerCallbackQuery(q.id).catch((error) => {
    console.warn(`[Model UI] answerCallbackQuery 실패: ${error.message}`);
  });

  return renderModelsForProvider(bot, {
...
async function refreshModels(bot, q, providerName) {
  ...
  await bot.answerCallbackQuery(q.id, { text: '백그라운드 모델 조회를 시작합니다.' }).catch((error) => {
    console.warn(`[Model Refresh] answerCallbackQuery 실패: ${error.message}`);
  });

  await editModelMessage(
    bot,
    chatId,
    messageId,
    `⏳ **[${providerName.toUpperCase()}] 모델 목록 갱신 중...**\n\n기존 캐시는 유지됩니다.`,
    { inline_keyboard: [] }
  );
...
async function showSuccess(bot, q, providerName, modelId) {
  ...
  return editModelMessage(
    bot,
    q.message.chat.id,
    q.message.message_id,
    `✅ **모델 설정 완료**\n\n📌 **세션**: **${active.title}**\n🤖 **Provider**: \`${providerName.toUpperCase()}\`\n🧠 **Model**: \`${modelId}\``,
    { inline_keyboard: [[{ text: '🔄 다른 모델로 변경', callback_data: `model_provider:${providerName}` }]] }
  );
}
...
export async function handleModelCallback(bot, q) {
  ...
  if (data === 'model_back_to_providers') {
    await handleModelCommand(bot, q);
...
async function editModelMessage(bot, chatId, messageId, text, replyMarkup) {
  try {
    ...
    console.error(`[Model UI] Telegram editMessageText 실패: chat=${chatId}, message=${messageId}, error=${error.message}`);
    throw error;
  }
}
